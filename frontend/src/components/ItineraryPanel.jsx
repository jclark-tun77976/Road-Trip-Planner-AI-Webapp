...
function ItineraryPanel({ response }) {
  ...
  const [isRouteLegsOpen, setIsRouteLegsOpen] = useState(false);
  ...
  return (
    <div className="result-card itinerary-card">
      <button
        ...
        onClick={() => setIsItineraryOpen((previous) => !previous)}
      ...
      >
        ...
        <span className="collapse-icon">{isItineraryOpen ? "▾" : "▸"}</span>
      </button>
      ...
      <button
        type="button"
        className="collapse-toggle"
        onClick={() => setIsRouteLegsOpen((previous) => !previous)}
        aria-expanded={isRouteLegsOpen}
      >
        <div className="section-header">
          <h3>Route Legs</h3>
          {response.route && (
        ...
        <span className="collapse-icon">{isRouteLegsOpen ? "▾" : "▸"}</span>
      </button>
